...
//        param.addValue("furigana",inputData.getFurigana());
    ...
    public List<CommunityThread> findKeyThread(String[] keywords,Integer communityId) {
        ...
        for (String key : keywords) {
            ...
            var list = jdbcTemplate.query("select *\n" +
                            ...
                            ") h\n" +
                            "on h.thread_id = t.id\n" +
                            "where t.community_id= :community_id AND t.title LIKE :key ORDER BY t.id", param,
                    new DataClassRowMapper<>(CommunityThread.class));
